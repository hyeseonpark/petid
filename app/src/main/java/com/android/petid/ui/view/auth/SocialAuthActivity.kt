...
//                    Toast.makeText(this, "Firebase 인증에 실패했습니다.", Toast.LENGTH_SHORT).show()
    ...
    private fun doLogin() {
        lifecycleScope.launch {
            try {
                ...
                val result = socialAuthViewModel.login(sub, fcmToken)
                handleLoginResult(result)
            } catch (e: Exception) {
                // 에러 처리
                Log.e("SocialAuthActivity", "Login failed", e)
            }
        }
    }

    private fun handleLoginResult(result: ApiResult<AuthEntity>) {
        when (result) {
            is ApiResult.Success -> {
                // 로그인 성공 처리
                val loginEntity = result.data
                Log.d("SocialAuthActivity", "Login successful: ${loginEntity.accessToken}")
            }
            is ApiResult.Error -> {
    ...
    private fun handleLoginError(error: Throwable) {
        ...
        try {
            // 에러 메시지 파싱 (JSON 형식 가정)
            val jsonError = JSONObject(errorMessage)
            val status = jsonError.getInt("status")
            val errorDescription = jsonError.getString("error")

            if (status == 404 && errorDescription.startsWith("Member UID")) {
                // 회원가입 화면으로 이동
                goTermsActivity()
            } else {
                // 일반 에러 처리
                Log.e("SocialAuthActivity", "Login error: $errorDescription")
            }
        } catch (e: JSONException) {
            // JSON 파싱 에러 처리
            Log.e("SocialAuthActivity", "Failed to parse error message", e)
        }
    }
